...
function showDate(dia, mes, ano) {
    document.write(`dia ${dia} <br> mes ${mes} <br> ano ${ano}`)    
}

function action() {
    let listDate = document.getElementById('data').value
   let [dia, mes, ano] = formatDate(listDate)

    let answer = dateIsValid(dia, mes, ano)
    if(!answer){
        alert('Data Invalida')
    } else {
        showDate(dia, mes, ano)
    }
